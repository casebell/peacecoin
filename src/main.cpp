...
#if defined(NDEBUG)
# error "Peacecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x668d8896f24330bcf99cc1932e77b42351e37518b43bb7f84048ba4ec5798e44");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Peacecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Peacecoin Signed Message:\n";
...
{
    // Peacecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Peacecoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 5 * 60; // Peacecoin: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // Peacecoin: .5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Peacecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Peacecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x668d8896f24330bcf99cc1932e77b42351e37518b43bb7f84048ba4ec5798e44");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 15/Jan/2017 Trump's E.P.A. Pick Backed Industry Donors Over Regulators";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xec934a97fa8be56bfca6f733960d69616e6094b9d5c3229b27ba8565c82358cd"));
...
unsigned char pchMessageStart[4] = { 0xbf, 0xfb, 0xde, 0xad }; // Peacecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PeacecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PeacecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PeacecoinMiner(CWallet *pwallet)
{
    printf("PeacecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("peacecoin-miner");
    ...
    try { loop {
        ...
        printf("Running PeacecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PeacecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PeacecoinMiner, pwallet));
}
